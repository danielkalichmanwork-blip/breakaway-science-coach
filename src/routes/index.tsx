    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          <a href="#top" className="flex items-center">
            <img src={logoWhite} alt="Breakaway Method" width={180} height={41} className="h-9 w-auto" />
          </a>
          ...
          <a href="#contact" className="inline-flex items-center gap-1.5 rounded-sm bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">
            Start coaching <ArrowUpRight className="size-4" />
          </a>
        </nav>
      </header>
      ...
      <section id="top" className="relative flex min-h-[92vh] items-end overflow-hidden">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-5 pb-20 pt-32">
          ...
          <h1 className="mt-4 max-w-3xl text-5xl leading-[0.95] uppercase sm:text-6xl lg:text-7xl">
            Train on evidence. <br />
            <span className="text-primary">Race on form.</span>
          </h1>
          <p className="mt-6 max-w-xl text-lg text-muted-foreground">
            Breakaway Method is one-on-one cycling coaching built around measured physiology and measured aerodynamics — for riders who want to know why they're faster.
          </p>
          <div className="mt-9 flex flex-wrap items-center gap-3">
            <a href="#contact" className="inline-flex items-center gap-2 rounded-sm bg-primary px-6 py-3 font-semibold text-primary-foreground transition-opacity hover:opacity-90 glow-primary">
              Book a free consultation <ArrowUpRight className="size-4" />
            </a>
            <a href="#method" className="inline-flex items-center gap-2 rounded-sm border border-border px-6 py-3 font-semibold transition-colors hover:bg-secondary">
              See the method
            </a>
          </div>
          <dl className="mt-14 grid max-w-2xl grid-cols-2 gap-x-8 gap-y-6 border-t border-border pt-8 sm:grid-cols-3">
            {[
              ...
              <div key={s.k}>
                <dt className="font-[family-name:var(--font-display)] text-2xl font-extrabold text-primary">{s.k}</dt>
      ...
      <section id="testing" className="border-t border-border bg-card/40 py-24">
        <div className="mx-auto max-w-6xl px-5">
          ...
          <p className="mt-5 max-w-2xl text-muted-foreground">
            Guessing is expensive. Testing is not. Both services are available to coached athletes and as standalone bookings.
          </p>
            ...
            {[
              {
                ...
                points: ["Immediate A/B comparison of positions", "Helmet, skinsuit and wheel selection", "Watts and time saved quantified per change"],
              },
              {
                img: "/lactate-bw.jpg",
                ...
                points: ["True LT1 / LT2 identification", "Individualised training zones", "Fuelling and pacing targets"],
              },
            ].map((t) => (
            ...
            {PROCESS.map((s) => (
              <li key={s.step} className="border-t-2 border-primary pt-6">
                <div className="flex items-center gap-3">
                  <span className="font-[family-name:var(--font-display)] text-3xl font-black text-primary/40">{s.step}</span>
        ...
        <div className="mx-auto grid max-w-6xl gap-14 px-5 lg:grid-cols-[1fr_0.85fr]">
          <div>
            ...
            <div className="mt-6 space-y-4 text-muted-foreground">
              <p>
                I’m an ex-pro cyclist and now a coach who believes the best results derive from both experience and evidence. Everything I ask of an athlete, I’ve tested myself — through training, racing, wind, fatigue, and high-pressure competition.
              </p>
              <p>
                Breakaway Method was built around a simple idea: replace inherited training habits with measurable, individualized coaching. I use data including lactate, power, aerodynamics, training load, and race performance to turn physiology and sports science into practical decisions.
              </p>
              <p>
                I coach road, gravel, time trial, and track cyclists — from developing racers to athletes pursuing national-level performance. The approach is always individual. The standard of evidence is not.
              </p>
            </div>
          </div>
          <div className="rounded-sm border border-border bg-card p-8">
            ...
            <div className="mt-8 border-t border-border pt-6">
              ...
              <p className="mt-3 text-sm text-muted-foreground">Position work is only as good as the fit underneath it. Breakaway Method athletes work with FT Cycling for professional bike fitting.</p>
              <a href="https://www.ftcycling.ca/" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline">
                FT Cycling bike fitting <ArrowUpRight className="size-4" />
              </a>
            </div>
          </div>
        </div>
      ...
      <section id="resources" className="border-t border-border py-24">
        <div className="mx-auto max-w-6xl px-5">
          ...
          <p className="mt-5 max-w-2xl text-muted-foreground">Coaching notes on training, physiology and racing — the why behind the work.</p>
          <a href="https://substack.com/@breakawaymethod" target="_blank" rel="noopener noreferrer" className="mt-12 block overflow-hidden rounded-sm border border-border bg-card shadow-lift transition-colors hover:border-primary">
            <img
            ...
            <div className="flex items-center justify-between gap-4 p-6">
              <div>
                ...
                <p className="mt-2 text-sm text-muted-foreground">Read the latest posts on training, testing and racing.</p>
              </div>
              <span className="inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-primary">
                Read the latest posts <ArrowUpRight className="size-4" />
              </span>
            </div>
          </a>
        </div>
      ...
      <section id="contact" className="relative overflow-hidden border-t border-border py-24">
        ...
        <div className="relative mx-auto max-w-3xl px-5 text-center">
          ...
          <p className="mt-5 text-muted-foreground">Tell me about your season, your schedule and your goal. Free 30-minute consultation, no obligation.</p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            <a href="mailto:breakawaymethod@gmail.com?subject=Coaching%20enquiry" className="inline-flex items-center gap-2 rounded-sm bg-primary px-6 py-3 font-semibold text-primary-foreground transition-opacity hover:opacity-90 glow-primary">
              breakawaymethod@gmail.com <ArrowUpRight className="size-4" />
            </a>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-border py-10">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-5 text-sm text-muted-foreground sm:flex-row">
          <div className="flex items-center gap-3">
            <img src={logoWhite} alt="Breakaway Method" width={150} height={34} className="h-7 w-auto" />
            <span>— cycling coaching by Daniel Kalichman</span>
          </div>
          <a href="https://www.ftcycling.ca/" target="_blank" rel="noopener noreferrer" className="hover:text-foreground">
